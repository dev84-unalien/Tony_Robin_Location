...
if (empty($appart) || empty($email) || empty($name) || empty($surname) || empty($address) || empty($tel)) {
...
$requete = $connection->prepare("INSERT INTO `utilisateurs` (`appart`, `email`, `name`, `surname`, `address`, `tel`) VALUES (?, ?, ?, ?, ?, ?)");

if ($requete) {
    // Ajouter les valeurs a la requete SQL
    $requete->bind_param("sssssi", $appart, $email, $name, $surname, $address, $tel);
    $requete->execute();
